...
module.exports = {
  ...
  onStart: async function ({ api, event, args, message }) {
    ...
    try {
      // Step 1: Try current live matches
      ...
      // Filter logic
      if (query && query !== "auto") {
        matches = matches.filter(m => m.name.toLowerCase().includes(query));
        
        // Step 2: Fallback to Fixtures if no live matches for the query
        if (matches.length === 0) {
          endpoint = `https://api.cricapi.com/v1/matches?apikey=${API_KEY}&offset=0`;
          res = await axios.get(endpoint);
          matches = res.data.data.filter(m => m.name.toLowerCase().includes(query));
        }
      }

      if (!matches || matches.length === 0) return message.reply(`❌ No matches found for "${query ? query.toUpperCase() : "LIVE"}".${footer}`);

      // Step 3: Categorize matches by Series Name
      const categorized = {};
      matches.forEach(m => {
