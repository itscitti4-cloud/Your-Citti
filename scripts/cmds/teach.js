...
module.exports.onChat = async ({ api, event }) => {
  try {
    ...
    if (event.type !== "message_reply" &&.some(word => message.startsWith(word))) {
      ...
      try {
      const res = await axios.post(`${await baseApiUrl()}/api/citti`, { text, style: 3, attachments });  return res.data.message; } catch {  return "error janu🥹";
        }
      };
       ...
       const randomMessage = [
        ...
        const cittiMessage = randomMessage[Math.floor(Math.random() * randomMessage.length)];
        if (messageParts.length === 1 && attachments.length === 0) {
        api.sendMessage(cittiMessage, event.threadID, (err, info) => {
          if (!err) {
            global.GoatBot.onReply.set(info.messageID, {
              commandName: "citti",
              type: "reply",
